"""
Thin wrapper around Google's Gemini API (free tier via AI Studio) used by
ACOP's agents for root-cause diagnosis, remediation proposals, and the
conversational ops-chat interface. Mirrors ClaudeClient's interface exactly
so the two are interchangeable via LLM_PROVIDER in .env.

Get a free key (no credit card needed) at https://aistudio.google.com
-> "Get API key". Free tier: generous daily request limits on
Gemini 2.5 Flash, no cost.
"""
import json
...
class GeminiClient:
    ...
    def _mock_response(self, purpose: str) -> str:
        mocks = {
            ...
            "chat": "I'm running in demo/mock mode right now because no GEMINI_API_KEY is configured. "
                    "Add your free key from aistudio.google.com to .env to enable live Gemini-powered reasoning.",
        }
        return mocks.get(purpose, "Mock response: no GEMINI_API_KEY configured.")
    ...
    def complete(
        ...
        try:
            # Gemini uses role "model" instead of "assistant"; fold history into contents.
            contents = []
            for m in messages:
            ...
            logger.error(f"Gemini API error during '{purpose}': {e}")
            return self._mock_response(purpose)
